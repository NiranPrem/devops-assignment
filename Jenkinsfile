pipeline {
    agent any

    environment {
        API_IMAGE  = "devops-assignment/api"
        WEB_IMAGE  = "devops-assignment/web"
        IMAGE_TAG  = "latest"
        TRIVY_PATH = "/usr/local/bin/trivy"
    }

    options {
        timestamps()
        timeout(time: 30, unit: 'MINUTES')
        buildDiscarder(logRotator(numToKeepStr: '10'))
    }

    stages {

        stage('Checkout') {
            steps {
                checkout scm

                script {
                    env.IMAGE_TAG = sh(
                        script: 'git rev-parse --short HEAD',
                        returnStdout: true
                    ).trim()

                    echo "Building Git SHA: ${env.IMAGE_TAG}"
                    currentBuild.displayName = "#${env.BUILD_NUMBER}-${env.IMAGE_TAG}"
                }
            }
        }

        stage('Environment Check') {
            steps {
                sh '''
                    echo "=== User ==="
                    whoami

                    echo "=== Docker ==="
                    docker --version

                    echo "=== Kubectl ==="
                    kubectl version --client

                    echo "=== Trivy ==="
                    trivy --version

                    echo "=== Cluster ==="
                    kubectl get nodes
                '''
            }
        }

        stage('Build API Image') {
            steps {
                sh """
                    docker build \
                      -t ${API_IMAGE}:${IMAGE_TAG} \
                      -t ${API_IMAGE}:latest \
                      ./app/api
                """
            }
        }

        stage('Build Web Image') {
            steps {
                sh """
                    docker build \
                      -t ${WEB_IMAGE}:${IMAGE_TAG} \
                      -t ${WEB_IMAGE}:latest \
                      ./app/web
                """
            }
        }

        stage('Trivy Security Scan') {
            steps {
                script {

                    echo "Scanning API image..."

                    sh """
                        trivy image \
                          --severity HIGH,CRITICAL \
                          --format json \
                          --output trivy-api.json \
                          --no-progress \
                          ${API_IMAGE}:${IMAGE_TAG}
                    """

                    echo "Scanning WEB image..."

                    sh """
                        trivy image \
                          --severity HIGH,CRITICAL \
                          --format json \
                          --output trivy-web.json \
                          --no-progress \
                          ${WEB_IMAGE}:${IMAGE_TAG}
                    """

                    archiveArtifacts(
                        artifacts: 'trivy-*.json',
                        allowEmptyArchive: true
                    )

                    def apiHigh = sh(
                        script: '''
                            jq '[.Results[].Vulnerabilities[]? | select(.Severity=="HIGH")] | length' trivy-api.json
                        ''',
                        returnStdout: true
                    ).trim().toInteger()

                    def apiCritical = sh(
                        script: '''
                            jq '[.Results[].Vulnerabilities[]? | select(.Severity=="CRITICAL")] | length' trivy-api.json
                        ''',
                        returnStdout: true
                    ).trim().toInteger()

                    def webHigh = sh(
                        script: '''
                            jq '[.Results[].Vulnerabilities[]? | select(.Severity=="HIGH")] | length' trivy-web.json
                        ''',
                        returnStdout: true
                    ).trim().toInteger()

                    def webCritical = sh(
                        script: '''
                            jq '[.Results[].Vulnerabilities[]? | select(.Severity=="CRITICAL")] | length' trivy-web.json
                        ''',
                        returnStdout: true
                    ).trim().toInteger()

                    int totalHigh = apiHigh + webHigh
                    int totalCritical = apiCritical + webCritical

                    echo "API -> HIGH=${apiHigh}, CRITICAL=${apiCritical}"
                    echo "WEB -> HIGH=${webHigh}, CRITICAL=${webCritical}"
                    echo "TOTAL -> HIGH=${totalHigh}, CRITICAL=${totalCritical}"

                    if (totalCritical > 0) {
                        error("Security Gate Failed: ${totalCritical} CRITICAL vulnerabilities found.")
                    }

                    // Allow only ONE HIGH vulnerability across both images
                    if (totalHigh > 2) {
                        error("Security Gate Failed: ${totalHigh} HIGH vulnerabilities found. Maximum allowed is 2.")
                    }

                    echo "Security Gate PASSED"
                }
            }
        }

        stage('Import Images to k3s') {
            steps {
                sh """
                    docker save ${API_IMAGE}:${IMAGE_TAG} -o api-${IMAGE_TAG}.tar
                    docker save ${WEB_IMAGE}:${IMAGE_TAG} -o web-${IMAGE_TAG}.tar

                    docker save ${API_IMAGE}:latest -o api-latest.tar
                    docker save ${WEB_IMAGE}:latest -o web-latest.tar

                    sudo k3s ctr images import api-${IMAGE_TAG}.tar
                    sudo k3s ctr images import web-${IMAGE_TAG}.tar

                    sudo k3s ctr images import api-latest.tar
                    sudo k3s ctr images import web-latest.tar

                    sudo k3s ctr images list | grep devops-assignment || true

                    rm -f api-*.tar web-*.tar
                """
            }
        }

        stage('Deploy DEV') {
            steps {
                sh """
                    cd k8s/overlays/dev

                    kustomize edit set image \
                      ${API_IMAGE}=${API_IMAGE}:${IMAGE_TAG}

                    kustomize edit set image \
                      ${WEB_IMAGE}=${WEB_IMAGE}:${IMAGE_TAG}

                    cd ../../..

                    kubectl apply -k k8s/overlays/dev

                    kubectl rollout restart deployment/api -n dev
                    kubectl rollout restart deployment/web -n dev
                """
            }
        }

        stage('Validate DEV') {
            steps {
                sh """
                    kubectl rollout status deployment/api -n dev --timeout=180s
                    kubectl rollout status deployment/web -n dev --timeout=180s

                    kubectl get pods -n dev -o wide
                    kubectl get svc -n dev
                """
            }
        }

        stage('Deploy QAT') {
            steps {
                sh """
                    cd k8s/overlays/qat

                    kustomize edit set image \
                      ${API_IMAGE}=${API_IMAGE}:${IMAGE_TAG}

                    kustomize edit set image \
                      ${WEB_IMAGE}=${WEB_IMAGE}:${IMAGE_TAG}

                    cd ../../..

                    kubectl apply -k k8s/overlays/qat

                    kubectl rollout restart deployment/api -n qat
                    kubectl rollout restart deployment/web -n qat
                """
            }
        }

        stage('Validate QAT') {
            steps {
                sh """
                    kubectl rollout status deployment/api -n qat --timeout=180s
                    kubectl rollout status deployment/web -n qat --timeout=180s

                    kubectl get pods -n qat -o wide
                    kubectl get svc -n qat
                """
            }
        }

        stage('Final Status') {
            steps {
                sh '''
                    echo "===== DEV ====="
                    kubectl get all -n dev

                    echo ""
                    echo "===== QAT ====="
                    kubectl get all -n qat

                    echo ""
                    NODE_IP=$(kubectl get nodes -o jsonpath='{.items[0].status.addresses[?(@.type=="InternalIP")].address}')

                    echo "DEV Web : http://${NODE_IP}:30080"
                    echo "QAT Web : http://${NODE_IP}:30081"
                '''
            }
        }
    }

    post {

        success {
            echo "SUCCESS: Build ${IMAGE_TAG} deployed to DEV and QAT"
        }

        failure {
            echo "FAILED - initiating rollback"

            sh '''
                kubectl rollout undo deployment/api -n dev 2>/dev/null || true
                kubectl rollout undo deployment/web -n dev 2>/dev/null || true

                kubectl rollout undo deployment/api -n qat 2>/dev/null || true
                kubectl rollout undo deployment/web -n qat 2>/dev/null || true

                echo "Rollback complete"

                kubectl get pods -n dev || true
                kubectl get pods -n qat || true
            '''
        }

        always {
            sh 'docker image prune -f --filter "until=24h" || true'
            deleteDir()
        }
    }
}
